          ...
          .map((session) => (
            <li key={session.id}>
              <Link
                ...
                className="block rounded border border-zinc-800 bg-zinc-900 p-4 hover:border-emerald-600"
              >
                <div className="flex items-center justify-between">
                  <span className="font-medium text-white">{session.date}</span>
                  <span className="text-sm text-zinc-500">{session.sets.length} sets</span>
                </div>
                {session.notes && (
                  <p className="mt-1 text-sm text-zinc-400">{session.notes}</p>
                )}
              </Link>
            </li>
          ))}
